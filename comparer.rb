...
outputs to filename1-filename2-unique and filename2-filename1-unique
...
class Comparer
  ...
  def dump_remainders()
    ...
    r1 = File.expand_path("diff-#{base1}-#{base2}", File.dirname(@file1))
    r2 = File.expand_path("diff-#{base2}-#{base1}", File.dirname(@file2))
    File.open(r1, 'w') do |f|
      @unique1.each do |q|
        f.puts("#{q[1]} #{q[0]}")
      end
